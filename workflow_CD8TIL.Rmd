---
title: "Analysis of sc-RNAseq data (smart-seq2) of CD8 TIL from B16 tumors of Singer et al Cell 2016"
...
We use a public dataset of small size (~1K cells) in order to make fast calculations
...
```{r}
data.seurat@hvg.info[order(data.seurat@hvg.info$gene.mean,decreasing = T),]
```
...
Strong effect from cell cycle genes,
...
```{r}
clusterColors <- c("#F8766D","#A58AFF","#00B6EB","#53B400","#000000")
...
cd8.markers <- FindAllMarkers(object = data.seurat, only.pos = TRUE, min.pct = 0.1, min.diff.pct=0.1, logfc.threshold = 0.25, test.use = "MAST",max.cells.per.ident = Inf)
```
...
```{r}
signatureList <- list()
signatureList$gs_Cytotoxicity <- c("Prf1","Fasl","Gzmb")
signatureList$gs_Stemness <- c("Tcf7","Sell","Il7r","Lef1")
signatureList$gs_InhibitoryReceptors <- c("Pdcd1","Havcr2","Tigit","Lag3","Ctla4")
```
...
```{r, fig.width=2}
for (s in grep("gs_",names(signatureList), value=T)) {
